...
void loop() {
  ...
  // add your code here!
  float dist_cm = dist_ema / 10.0;     
  float angle_deg;
  if (dist_cm <= 18.0) angle_deg = 0.0;
  else if (dist_cm >= 36.0) angle_deg = 180.0;
  else angle_deg = ((dist_cm - 18.0) / 18.0) * 180.0; 

  int duty = map(angle_deg, 0, 180, _DUTY_MIN, _DUTY_MAX); 
